...
face_detector = dlib.get_frontal_face_detector()   # fallback when Haar misses
...
while True:
    ...
    gray     = np.array(gray_raw, dtype=np.uint8)   # plain uint8 array dlib accepts



    # Primary: Haar Cascade (as per project spec)
    ...
    # Fallback: dlib HOG when Haar finds nothing
    ...
    if len(haar_faces) == 0:
        ...
        faces = [dlib.rectangle(int(x), int(y), int(x+w), int(y+h))
